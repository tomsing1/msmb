...
source("../chapter-setup.R"); chaptersetup("/home/msmith/Projects/CUBook/Chap5-Mixtures/MixtureModel.Rnw", "5")
...
## ----chap5-r-plotvstgammapoisson-1, fig.keep = 'high', fig.cap = "Graph of the function \\@ref(eq:mixtures-vstgammapoisson) for different choices of $\\alpha$.", fig.width = 4.2, fig.height = 4----
f = function(x, a) 
    1/(2*sqrt(a)) * log(2 * sqrt(a) * sqrt(a*x^2+x) + 2*a*x + 1)
x = seq(0, 24, by = 0.1)
df = lapply(c(0.1, 0.2, 0.3), function(a) 
    tibble(x = x, y = f(x, a), what = paste0("a=", a))) %>% 
    bind_rows(tibble(x = x, y = sqrt(x), what = "sqrt"))
ggplot(df, aes(x = x, y = y, col = what)) + geom_line() + 
    geom_point(size = 0.5 * (df$x %% 1 == 0)) + 
...
## ----chap5-r-EMillustrate-1, fig.keep = 'high', fig.cap = "Histogram of our example dataset for the EM algorithm.", fig.width = 2.5, fig.height = 2.5----
