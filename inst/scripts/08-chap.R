...
source("../chapter-setup.R"); chaptersetup("/home/msmith/Projects/CUBook/Chap7-RNAseq/rnaseqanalysis.Rnw", "7")
...
### The two lines commented out below could be used to reproduce Mike Love's way of selecting the two genes: they should have similar
...
## ----rnaseq-posterior, fig.keep = 'high', fig.show = "hold", fig.cap = "Shrinkage estimation of logarithmic fold change estimates by use of an empirical prior in **[DESeq2](https://bioconductor.org/packages/DESeq2/)**. Two genes with similar mean count and MLE logarithmic fold change are highlighted in green and blue. The normalized counts for these genes (upper panel) reveal low dispersion for the gene in blue and high dispersion for the gene in green. In the lower panel, the density plots are shown of the normalized likelihoods (solid lines) and of the posteriors (dashed lines) for the green and blue gene. In addition, the solid black line shows the prior estimated from the MLEs of all genes. Due to the higher dispersion of the green gene, its likelihood is wider and less sharp (indicating less information), and the prior has more influence on its posterior than in the case of the blue gene.", fig.width = 3.2, fig.height = 2.2, echo = FALSE, results = "hide"----
library("ggbeeswarm")
...
## ----rnaseq-exbatch, fig.keep = 'high', fig.cap = "p-values for the tests performed on `x1` and `x2` (see code).", fig.width = 3, fig.height = 5----
library("magrittr")
ng = 10000
ns = 12
